...
function _convertNH(node::treecausation_3_opti_xgboost.NodeMeta{S}, indX::Vector{Int}) where {S}
    if node.is_leaf
...
function _convertH(node::treecausation_3_opti_xgboost.NodeMeta{S}, indX::Vector{Int}) where {S}
    if node.is_leaf
...
function fill_treeH(
        node     :: treecausation_3_opti_xgboost.NodeMeta{S},
...
function build_tree_3_opti_xgboost(
        honest             :: Bool,
    ...
    t = treecausation_3_opti_xgboost.fit(
        X                   = features,
        Y                   = labels,
        W                   = treatment,
        indX                = indsbuild,
...
"""
function build_forest_3_opti_xgboost(
    centering          :: Bool,
    bootstrap          :: Bool,
    honest             :: Bool,
    labels             :: AbstractVector{T},
    ...
    model_Y = XGBoost.xgboost(features[treatment.==0,:], 2, label = labels[treatment.==0], eta = 0.3,  max_depth = 6)
    Y_center = labels - XGBoost.predict(model_Y, features)
    Y_vec = Y_center
    T_vec = treatment
    ...
    if rng isa Random.AbstractRNG
        if bootstrap
            Threads.@threads for i in 1:n_trees
                ...
                forest[i] = build_tree_3_opti_xgboost(
                    honest,
                    indsbuild,
                    indspred,
                    Y_vec,
...
function apply_forest_3_xgboost(
...
function apply_forest_3_xgboost(
    ...
    for i in 1:N
        predictions[i] = apply_forest_3_xgboost(forest, x[i, :])
    end
    return predictions
end
