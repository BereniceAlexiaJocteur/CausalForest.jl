...
function _convertNH(node::treecausation_3_opti.NodeMeta{S}, indX::Vector{Int}) where {S}
    if node.is_leaf
...
function _convertH(node::treecausation_3_opti.NodeMeta{S}, indX::Vector{Int}) where {S}
    if node.is_leaf
...
function fill_treeH(
        node     :: treecausation_3_opti.NodeMeta{S},
...
function build_forest_3_opti(
    ...
    println("Erreur du modèle préliminaire")
    println(StatsBase.rmsd(labels[treatment.==0], DecisionTree.apply_forest(model_Y, features[treatment.==0,:])))
...
function apply_forest_3(
    ...
    for i in 1:N
        predictions[i] = apply_forest_3(forest, x[i, :])
    end
    return predictions
end
