function _convertNH(node::treecausation_opti.NodeMeta{S}, indX::Vector{Int}) where {S}
    if node.is_leaf
...
function _convertH(node::treecausation_opti.NodeMeta{S}, indX::Vector{Int}) where {S}
    if node.is_leaf
...
function fill_tree(
        node     :: treecausation_opti.NodeMeta{S},
