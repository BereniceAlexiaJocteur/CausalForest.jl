function _convertNH(node::treecausation_2.NodeMeta{S}, indX::Vector{Int}) where {S}
    if node.is_leaf
...
function _convertH(node::treecausation_2.NodeMeta{S}, indX::Vector{Int}) where {S}
    if node.is_leaf
...
function _calculate_labels!(
        node     :: LeafOrNodeCausalH{S},
        X        :: AbstractMatrix{S},
        Y        :: Vector{Float64},
        W        :: Vector{Int}) where {S}

    if is_leaf(node)
        n = length(node.inds_pred)
        t_w_sum = zero(Int)
        t_y_sum = zero(Float64)
        t_wy_sum = zero(Float64)
        ...
        node.label = t_wy_sum/t_w_sum - (t_y_sum-t_w_sum)/(n-t_w_sum)
    else
        if X[node.featid] < node.featval
            _calculate_labels!(node.left, X, Y, W)
        else
            _calculate_labels!(node.right, X, Y, W)
        end
    end
end
...
function fill_treeH(
        node     :: treecausation_2.NodeMeta{S},
...
function fill_treeNH(
    ...
    _calculate_labels!(nodeNH, X, Y, T)
    return nodeNH
end


function build_tree_2(
        honest             :: Bool,
...
apply_tree_2(leaf :: Union{LeafCausalH, LeafCausalNH}, x :: AbstractVector{S}) where {S} = leaf.label

function apply_tree_2(tree :: Union{NodeCausalH{S}, NodeCausalNH{S}}, x :: AbstractVector{S}) where {S}
    if tree.featid == 0
        return apply_tree_2(tree.left, x)
    elseif x[tree.featid] < tree.featval
        return apply_tree_2(tree.left, x)
    else
        return apply_tree_2(tree.right, x)
    end
end

function apply_forest_2(
    ...
    res = 0
    for b in 1:n_trees
        res += apply_tree_2(forest.trees[b].tree, x)
    end
    return res/n_trees
end
...
function apply_forest_2(
    ...
    for i in 1:N
        predictions[i] = apply_forest_2(forest, x[i, :])
    end
    return predictions
end


function apply_forest_2bis(
    forest :: EnsembleCausal{S},
    x      :: AbstractVector{S}
    ) where {S}
...
function apply_forest_2bis(
    ...
    predictions = Array{Float64}(undef, N)
    for i in 1:N
        predictions[i] = apply_forest_2bis(forest, x[i, :])
    end
    return predictions
end
