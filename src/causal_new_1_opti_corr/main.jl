function _convertNH(node::treecausation_1_opti_corr.NodeMeta{S}, indX::Vector{Int}) where {S}
    if node.is_leaf
...
function _convertH(node::treecausation_1_opti_corr.NodeMeta{S}, indX::Vector{Int}) where {S}
    if node.is_leaf
...
function fill_treeH(
        node     :: treecausation_1_opti_corr.NodeMeta{S},
...
function build_tree_1_opti_corr(
        honest             :: Bool,
    ...
    t = treecausation_1_opti_corr.fit(
        X                   = features,
        Y                   = labels,
        W                   = treatment,
        indX                = indsbuild,
...
"""
function build_forest_1_opti_corr(
    centering          :: Bool,
    bootstrap          :: Bool,
    honest             :: Bool,
    labels             :: AbstractVector{T},
    ...
    if rng isa Random.AbstractRNG
        if bootstrap
            Threads.@threads for i in 1:n_trees
                ...
                forest[i] = build_tree_1_opti_corr(
                    honest,
                    indsbuild,
                    indspred,
                    Y_vec,
